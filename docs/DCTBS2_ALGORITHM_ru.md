...
В таблице указано исходное фиксированное распределение:

| Режим | MCU | Y | Cb | Cr | Форма Y |
| ---: | ---: | ---: | ---: | ---: | --- |
| 0,75 bpp | 24 B | 8 B | 8 B | 8 B | 1 × 16×16 |
...
В режимах 0,75, 1, 1,5, 2 и 3 bpp по умолчанию работает быстрый адаптивный
подбор бюджета компонентов. Кодер создаёт несколько файлов одинакового размера
с разными долями Y/Cb/Cr, полностью декодирует их и оставляет вариант с
минимальной RGB-ошибкой. Исходное распределение также входит в кандидаты,
поэтому подбор не может ухудшить качество относительно прежней фиксированной
схемы. Расширенный режим проверяет больший диапазон бюджетов цветности.

Для 3 bpp основные кандидаты: `Y80 + Cb8 + Cr8`, `Y72 + Cb12 + Cr12`,
`Y64 + Cb16 + Cr16` и `Y56 + Cb20 + Cr20`. Энкодер всегда выделяет не менее
8 байт отдельно для Cb и Cr, чтобы адаптивное распределение не жертвовало
насыщенностью цвета ради дополнительных коэффициентов яркости. Поля Y/Cb/Cr
в заголовке являются
авторитетными; код режима задаёт только общий размер MCU. Сумма трёх полей
обязана совпадать с `bytesPerMcu`, а Y в раздельной схеме должен делиться на
четыре целых записи. Размер MCU и формула прямой адресации при этом не меняются.
Режимы выше 3 bpp и кодирование с библиотекой прототипов используют исходное
