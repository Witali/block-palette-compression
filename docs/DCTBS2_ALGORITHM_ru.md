...
> DCTBS2 не использует зигзаг JPEG. Grouped-запись использует четыре
> детерминированных профиля значимости, а skip-запись — восемь расширенных
> профилей. Профиль выбирается для каждого компонентного блока отдельно.
...
группы. В профилях 1,5, 3, 4,5 и 6 bpp используются три группы с более короткой
первой низкочастотной частью. Старый вариант с signed 6-bit AC и единым
масштабом сохраняется для совместимости.
...
Для 32-байтных записей 6 bpp сохранён grouped-вариант. В низких режимах skip
применяется к Y, а Cb/Cr остаются grouped; в высоких split-luma режимах skip
доступен всем трём компонентам.
...
до 6 bpp. Они поддерживают базовые grouped/legacy записи, adaptive skip-RLE и
dual-scale skip, разделённую Y и версии библиотеки 1…9. Адреса MCU, sidecar и
прототипа вычисляются напрямую.
