...
8×8 Y-блока, bit 2 — наличие библиотеки, bit 3 — новую цветность 4:2:0, а
bits 8…11 выбирают способ упаковки коэффициентов. Если bit 3 сброшен, декодер
...
mask/tail. В маске bit 0 соответствует первому AC `DCT[1]`, а DC хранится
отдельно; незанятые слоты образуют неявный непрерывный хвост. В 48-байтной
