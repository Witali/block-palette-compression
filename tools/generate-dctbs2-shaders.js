...
function renderUnrolledComponentDecoder(options) {
  ...
  const lines = [
    ...
    "",
    "  addDctCompensated(",
    `    float(dc) * exp2(float(dcScaleIndex)) * dctQuantizationStep(0, ${chroma}, ${height}) *`,
  ...
  for (let wordIndex = 0; wordIndex <= maximumWord; wordIndex += 1) {
    ...
    for (const { index, wordBit } of coefficientsByWord.get(wordIndex) || []) {
      ...
      lines.push(
        ...
        `        dctBasis(position >> ${positionShift}, localY, ${height}),`,
        "      sum,",
        "      correction",
        "    );",
        "  }"
...
${renderUnrolledComponentDecoder({
    functionName: "sampleDctChroma422Record",
    scanName: "DCT_SCAN_C422",
...
${renderUnrolledComponentDecoder({
    functionName: "sampleDctChroma420Record",
    scanName: "DCT_SCAN_C420",
    coefficientCount: chromaCount,
    width: 8,
    height: 8,
    chroma: "true",
...
function renderCubeFragmentShader(preset) {
  ...
  const chroma422Scan = renderCubeIntArray(
    "DCT_SCAN_C422", trimmedScans(8, 16, chromaCount)
  );
  const chroma420Scan = renderCubeIntArray(
    "DCT_SCAN_C420", trimmedScans(8, 8, chromaCount)
  );
...
function renderFragmentShader(preset) {
  ...
  const chroma422Scans = glslIntArray("SCAN_C422", allScans(8, 16));
  ...
  const chroma422SkipScans = glslIntArray("SKIP_SCAN_C422", allSkipScans(8, 16));
...
int groupCount(int coding) {
    ...
    return coding == 2 || coding == 5 || coding == 6 || coding == 7 ? 3 : 0;
}
...
float sampleImplicit2MaskedTailRecord(
    int recordOffset,
    int recordBytes,
    int localX,
    int localY,
    ...
    int bitOffset = 60;
    int scaleIndex = int(readSidecarBits(recordOffset, bitOffset, 2));
    bitOffset += 2;
    int dc = readSignedSidecarBits(recordOffset, bitOffset, 10);
    bitOffset += 10;
    float scale = exp2(float(scaleIndex));
    float sum = 0.0;
    float correction = 0.0;
    ...
    const int implicitPositions[2] = int[2](1, 8);
    for (int index = 0; index < 2; ++index) {
        int position = implicitPositions[index];
