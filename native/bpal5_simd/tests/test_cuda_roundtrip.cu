...
__device__ uint32_t sample_bpal_pixel(const uint8_t *bytes, uint32_t x, uint32_t y) {
    ...
    const uint32_t channel_mode = read_bits_at(bytes, 108u, 2u);
    const uint32_t packed = read_bits_at(bytes, 110u, 2u) & 1u;
    const uint32_t stored_color_bits = channel_mode == BPAL5_CHANNEL_SCALAR ? 8u : color_bits;
    ...
    const uint64_t selector_offset = packed != 0u
        ...
        : 112u + static_cast<uint64_t>(palette_count) * global_count * stored_color_bits;
    ...
    if (packed == 0u) {
        const uint32_t color = read_bits_at(
            bytes,
            112u + (static_cast<uint64_t>(palette) * global_count + global) * stored_color_bits,
            stored_color_bits
        );
        return channel_mode == BPAL5_CHANNEL_SCALAR
            ? color | color << 8u | color << 16u | 0xff000000u
            : color_bits == 16u
            ? unpack_rgb565(color)
...
int main() {
    ...
    return run_roundtrip(source, width, height, 24u, BPAL5_CHANNEL_RGB, false) != 0 ||
        run_roundtrip(source, width, height, 16u, BPAL5_CHANNEL_RGB, false) != 0 ||
        run_roundtrip(source, width, height, 16u, BPAL5_CHANNEL_SCALAR, false) != 0 ||
        run_roundtrip(narrow_source, width, height, 24u, BPAL5_CHANNEL_RGB, true) != 0;
}
