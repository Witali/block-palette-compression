...
| **DCTBS2** (`.dctbs2`) | 2 | A transform-coded alternative with independent `16x16` YCbCr 4:2:0 MCUs and fixed record sizes from 0.75 to 9 bpp. Higher-rate modes split luma into four `8x8` transforms; optional directly addressed prototype libraries reduce coefficient residuals. Legacy 4:2:2 files remain readable. | One fixed MCU record, its quantization metadata, and an optional prototype entry are sufficient for a pixel. Supported by the DCT page, the WebGL2 Cube shader, and the native CUDA tool. |
